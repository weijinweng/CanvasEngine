...
struct GenericPlatformTypes
{
	...
	typedef std::uint8_t			CHAR8;		// An 8-bit character type - In-memory only.  8-bit representation.  
	typedef std::uint16_t			CHAR16;		// A 16-bit character type - In-memory only.  16-bit representation.
	typedef std::uint32_t			CHAR32;		// A 32-bit character type - In-memory only.  32-bit representation.
	//typedef WIDECHAR				TCHAR;		// A switchable character  - In-memory only.  Either ANSICHAR or WIDECHAR
